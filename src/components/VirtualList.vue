...
const scrollHandler = () => {
  ...
  if (curScrollTop) {
    const addCount = Math.floor(curScrollTop / state.unitH);
    listRef.value?.style.setProperty(
      "padding-top",
      `${addCount * state.unitH}px`
    );
    console.log(addCount, addCount * state.unitH);
